...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(L):
        ...
        if i==0:
            ...
            if use_batchnorm:
                self.params['beta'+ string] = np.zeros(hidden_dims[i])
                self.params['gamma'+ string] = np.array([1])
        elif i!=L-1:
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
        ...
        #print('the w in the cache has dims: ', np.shape(cache[1]) )
        caches.append(cache)
        
        
        #   BATCH_NORM
        if self.use_batchnorm:
            gamma = self.params['gamma'+names[i]]
            beta  = self.params['beta'+names[i]]
        ...
        #print('now we do batchnorm_forward')
    ...
    for i in range(len(caches) - 2, -1, -3):
        ...
        if self.use_batchnorm:
            grads['gamma' + names[name_index] ] = dgamma
            grads['beta' + names[name_index] ] = dbeta
        name_index -= 1
        dout = dx
