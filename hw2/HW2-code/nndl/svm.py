...
class SVM(object):
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
        ...
        for j in np.arange(num_classes):
            if j!=y[i]:
                loss += max( 0, 1 + np.dot(self.W[j,:], X[i,:] ) - np.dot(self.W[y[i],:], X[i,:] ) )
                prefac += ( 1 + np.dot(self.W[j,:], X[i,:] ) - np.dot(self.W[y[i],:], X[i,:] )>0 )
           
        for j in np.arange(num_classes):
            if j!=y[i]:
                grad[j,:] += ( 1 + np.dot(self.W[j,:], X[i,:] ) - np.dot(self.W[y[i],:], X[i,:] )>0 )*X[i,:]
            else:
                grad[j,:] -= prefac * X[i,:]
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    alpha = X.dot(np.transpose(self.W))
    alpha_correct = alpha[np.arange(alpha.shape[0]),y] 
    margins = np.maximum(0, alpha - np.matrix(alpha_correct).T + 1)
    ...
    temp[np.arange(num_train), y] = -np.transpose(row_sum)
    grad = np.transpose(np.dot(np.transpose(X), temp))
    grad /= num_train
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
